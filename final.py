                ...
                for res, img in runner.classifier(videoCaptureDeviceId):
                    ...
                    if "bounding_boxes" in res["result"].keys():
                        for bb in res["result"]["bounding_boxes"]:
                            centroid_x = bb['x'] + bb['width'] // 2 - 140
...
def send_ned_velocity(vx, vy, vz, duration):
    ...
    loop_count = int(duration)
    extra_time = duration - loop_count
...
try:
    ...
    gsdy = (2*height*math.tan(hfov/2))/280
    gsdx = (2*height*math.tan(vfov/2))/280
    ...
    vxx = v*math.cos(theta_radians)
    vyy = v*math.sin(theta_radians)


    time.sleep(3)


    send_ned_velocity(vyy,vxx,0,tim)
